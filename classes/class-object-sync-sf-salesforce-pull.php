...
class Object_Sync_Sf_Salesforce_Pull {
		...
		foreach ( $this->mappings->get_fieldmaps( null, $this->mappings->active_fieldmap_conditions ) as $salesforce_mapping ) {
			...
			if ( ! isset( $response['errorCode'] ) && 0 < count( $response['records'] ) ) {
				...
				foreach ( $response['records'] as $key => $result ) {
				    //Make sure post_status gets set to publish
					//$result['Publish_To_Web__c'] = "publish";
	...
	private function get_pull_date_value( $type, $soql, $fieldmap_id = '' ) {
		...
		if ( $sf_last_sync ) {
			//Due a potential iming issue, look a few minutes back
			$sf_last_sync = $sf_last_sync - (3*60);
			$pull_trigger_field_value = gmdate( 'Y-m-d\TH:i:s\Z', $sf_last_sync );
		} else {
			//Due a potential iming issue, look a few minutes back
			$sf_activate_time = $sf_activate_time - (3*60);
	...
	private function get_merged_records() {
		...
		foreach ( $this->mergeable_record_types as $type ) {
			...
			foreach ( $mappings as $salesforce_mapping ) {
				//Due to potential timing issue looking back 3 minutes
				//$last_merge_sync = $this->pull_options->get( 'merge_last', $salesforce_mapping['salesforce_object'], '', time() - (3*60));
	...
	private function get_deleted_records() {
		...
		foreach ( $this->mappings->get_fieldmaps( null, $this->mappings->active_fieldmap_conditions ) as $salesforce_mapping ) {
			...
			// Iterate over each field mapping to determine our query parameters.
			foreach ( $mappings as $salesforce_mapping ) {
                //Due to potential timing issue, always look back a few minutes
				//$last_delete_sync = $this->pull_options->get( 'delete_last', $type, '', time() - (3* 60) );
	...
	/**
	 * Get unpublished records from salesforce.
	 *  Delete them from object_maps as well as WordPress
	 */
	private function get_unpublished_records (){
		//Get all events pulled in through this plugin
		$mappings = $this->mappings->get_all_object_maps();
		//Array used to collect the Salesforce Campaign IDs associated with the Wordpress events
		$Ids = array();
		//Trigger passed to the queue to indicate that a Wordpress post (event) will be deleted
		$sf_sync_trigger = $this->mappings->sync_sf_delete;
	
		// this sets the Salesforce object type, there is only one type right now so we do not need to loop through all of the field maps, used in deleting unpublished events
		$type = ($this->mappings->get_fieldmaps( null, $this->mappings->active_fieldmap_conditions ))[0]['salesforce_object'];  
		...
		//Setting up logging, used mainly for debugging
		if ( isset( $this->logging ) ) {
				$logging = $this->logging;
		} elseif ( class_exists( 'Object_Sync_Sf_Logging' ) ) {
				$logging = new Object_Sync_Sf_Logging( $this->wpdb, $this->version );
		...
		//Setting status and title of log associated with debugging
		$status = 'debug';
		$title  = sprintf( esc_html__('Testing unpublish function'), ucfirst( esc_attr( $status ) ));
		
		//Loop through the object map for Wordpress events pulled from Salesforce through this plugin
		foreach ($mappings as $obj) {
			foreach ($obj as $key => $value) {
				//Grab Salesforce Id associated with event
		...
		//Compose query for Salesforce looking for unpublished records using Ids of Campaign events already pulled into Wordpress
		//$query  = 'SELECT Id FROM Campaign WHERE Publish_To_Web__c = FALSE AND RecordTypeId IN (\'0125Y000002PfVzQAK\') AND Id IN (\'';
		//$query  = 'SELECT Id FROM Campaign WHERE Publish_To_Web__c != \'publish\' AND RecordTypeId IN (\'0125Y000002PfVzQAK\') AND Id IN (\'';
		$query  = 'SELECT Id FROM Campaign WHERE (Publish_To_Web__c != \'publish\' OR Status = \'Completed\' OR Status = \'Canceled\') AND RecordTypeId IN (\'0125Y000002PfVzQAK\') AND Id IN (\'';
		// (NOT Account__r.Name  like 'Test%')
		$query .= implode( '\', \'', $Ids);
		$query .= '\')';
		...
		//Check for errors and that records were returned
		if ( ! isset( $response['errorCode'] ) && 0 < count( $response['records'] ) ) {
			foreach ( $response['records'] as $key => $results ) {
				//Salesforce seriously returns Id for update requests and id for delete requests and this makes no sense but maybe one day they might change it somehow?
				if ( ! isset( $results['Id'] ) && isset( $results['id'] ) ) {
					$results['Id'] = $results['id'];
				}
				//Setup the Id and the deletedDate to pass to the queue
				$deleted_item = array(
					'Id'          => $results['Id'],
					'deletedDate' => gmdate( 'Y-m-d\TH:i:s\Z' )
				);

				/*Statements used to debug function
					$body = sprintf('<p>'. esc_html__('Adding to queue-') . esc_html__($deleted_item['Id']) .  esc_html__('-'));
					$body .= sprintf( esc_html__($deleted_item['deletedDate']) .'</p>');
					$body .= sprintf('<p>'. esc_html__('Type-') . esc_html__($type) .'</p>');
					$body .= sprintf('<p>'. esc_html__('SF sync trigger-') . esc_html__($sf_sync_trigger) .'</p>');
				...
				*/
				...
				// Add a queue action to delete data from WordPress after it has been unpublished in Salesforce.
				$this->queue->add(
					$this->schedulable_classes[ $this->schedule_name ]['callback'],
						array(
							'object_type'     => $type,
...
echo $output;
	...
	private function is_pull_allowed( $object_type, $object, $sf_sync_trigger, $salesforce_mapping, $map_sync_triggers ) {
		...
		// echo '<script>console.log("About to check stuff-' , $object ,  '")</script>';
		//debug_to_console( "about to read stuff" );
			//debug_to_console( $object );
				// Do not pull Campaign records where Publish to Web in SF has not been checked off
		/*add_filter( 'object_sync_for_salesforce_pull_object_allowed', 'check_publish_to_web', 10, 5 );
		// can always reduce this number if all the arguments are not necessary
		function check_publish_to_web( $pull_allowed, $object_type, $object ) {
			 echo '<script>console.log("About to check stuff2-' , $object ,  '")</script>';
			//echo "<script>console.log('This is what I am looking at Pub-' + $object[\'Name\'] + '-');</script>";
			//if ( 'Campaign' === $object_type && !$object['Publish_to_Web__c'] ) {
				//echo "<script>console.log('Dont publish ' + $object['Name']);</script>";
				//$pull_allowed = false;
			//$pull_allowed = $object["Publish_to_Web__c"];
			
			//}
			//else {
				//echo "<script>console.log(`Publishing ` + $object[\"Name\"]);</script>";
			//}
			return $pull_allowed;
		}*/
		return $pull_allowed;
