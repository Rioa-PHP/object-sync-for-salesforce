...
class Object_Sync_Sf_Salesforce_Push {
	...
	public function salesforce_push_sync_rest( $object_type, $object, $mapping, $sf_sync_trigger ) {
		...
		//Removing this since we will not be having new records pushed to Salesforce.
		//This section of code is causing a problem since a temporary id is created for Salesforce while information is being proccessed, but it is not being
		//removed from the database when the proccessing is completed. The temporary id is then causing an error wwhen we are querying for unpublished, canceled or 			//completed events in Salesforce.
		/*
