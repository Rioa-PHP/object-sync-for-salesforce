...
class Object_Sync_Sf_Salesforce_Select_Query {
	...
	public function __toString() {
		...
		if ( count( $this->conditions ) > 0 ) {
			...
			$query .=  ' WHERE Publish_to_Web__c = \'publish\' AND (Status != \'Completed\' OR Status != \'Canceled\') AND ' . implode( ' AND ', $where );
		}
		...
		// quick examp$query = str_replace(le to change the order to descending once the query is already a string.
		/*add_filter( 'object_sync_for_salesforce_pull_query_string_modify', 'change_pull_query_string', 10, 2 );
		function change_pull_query_string( $query, $query_parameters ) {
			//$query = str_replace( 'ASC', 'DESC', $query);
			//$query = str_replace('WHERE ', 'WHERE Publish_to_Web__c EQUALS 1 AND ', $query);
			return $query;
		}*/
