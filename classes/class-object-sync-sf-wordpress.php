...
class Object_Sync_Sf_WordPress {
	...
	private function post_create( $params, $id_field = 'ID', $post_type = 'post' ) {
		...
		//Only capture the following fields and add to database, other fields should be added using acf function to capture the appropriate metadata
		/* 'post_type'
            'post_title'
            'post_content'
            'post_status' 
		*/
			...
			$status = 'debug';
			$title  = sprintf(esc_html__( 'Metadata', 'object-sync-for-salesforce' ), 
							  ucfirst( esc_attr( $status ) )
			);
					$body = sprintf('<p>'. esc_html__('Putting together content:'));
					$body .= sprintf( '</p>');
		...
		foreach ( $params as $key => $value ) {
			if ( in_array( $value['method_modify'], $structure['content_methods'], true ) ) {
				//Modified to support use of acf functions
					$body .= sprintf('<p>'. esc_html__('Key:'));
					$body .= sprintf(esc_html__($key). '</p>');
					$body .= sprintf('<p>'. esc_html__('Value:'));
			        $body .= sprintf(esc_html__($value['value']). '</p>');
				//if(key == 'post_type' || key == 'post_title' || key == 'post_content' || key == 'post_status') {
					$content[ $key ] = $value['value'];
					unset( $params[ $key ] );
				//}
			}
		...
		if ( is_wp_error( $post_id ) ) {
			...
			//Use acf function to update the wp_postmeta and add the metadata
			/*$acf = realpath( __DIR__ . '../../../themes/php/admin/advanced-custom-fields-pro/acf.php');
			$body .= sprintf('<p>' . esc_html__( 'Found AFC:'));
			$body .= sprintf(esc_html__($acf)	. '</p>');
			if($acf) { 
				$body .= sprintf('<p>' . esc_html__( 'Really found AFC:'));
				$body .= sprintf(esc_html__($acf)	. '</p>');
            	require_once($acf);    
				...
				foreach ( $params as $key => $value ) {
					...
					$body .= sprintf('<p>'. esc_html__('Value:'));
			        $body .= sprintf(esc_html__($value['value']). '</p>');
			    	if(!str_contains($key, 'meta') || !str_contains($key, 'language')){
						if(!str_contains($key, 'field')){
							$body .= sprintf('<p>'. esc_html__('Adding single:'));
							$body .= sprintf(esc_html__($key). '</p>');
							update_field($key, $value['value'], $post_id);
						}
						else {
							$additonal_fields[ $key ] = $value['value'];
							$body .= sprintf('<p>'. esc_html__('Updating fields:'));
							$body .= sprintf(esc_html__($key). '</p>');
						}
					}
				}
				if (count($additional_fields) > 0) {
					update_field('additional_fields', $additional_fields, $post_id);
				}
			}*/

				// Original code used to update wp_postmeta
			...
			//At this point we are assuming the post has been successfully created
			//Let us set the language with the Polylang plugin
			//Polylang is loaded after this plugin, so let's load it if it has not already been loaded
			
			// Create log entry to debug
			/*if ( isset( $this->logging ) ) {
				$logging = $this->logging;
			} elseif ( class_exists( 'Object_Sync_Sf_Logging' ) ) {
				$logging = new Object_Sync_Sf_Logging( $this->wpdb, $this->version );
			...
			$status = 'debug';
			$title  = sprintf(esc_html__( 'Adding a language', 'object-sync-for-salesforce' ), 
							  ucfirst( esc_attr( $status ) )
			);*/
			
			$polylang = realpath( __DIR__ . '../../polylang/polylang.php');
			//$body = sprintf('<p>' . esc_html__( 'Found Polylang:'));
			$body .= sprintf(esc_html__($polylang)	. '</p>');
			
			if($polylang) { 
            	require_once($polylang); 
            }      
			
			$test = function_exists('pll_set_post_language');
			$test_lang = $params['event_language']['value'];
			$body .= sprintf('<p>'. esc_html__('Found pll function:'));
			$body .= sprintf(esc_html__($test). '</p>');
			$body .= sprintf('<p>'. esc_html__('Language:'));
			$body .= sprintf(esc_html__($test_lang) . '</p>');
			/*foreach ( $params as $key => $value ) {
				$body .= sprintf('<p>'. esc_html__('Key:'));
				$body .= sprintf(esc_html__($key) . '</p>');
				$body .= sprintf('<p>'. esc_html__('Value:'));
			...
			$body .= sprintf('<p>'. esc_html__('Post ID:'));
			$body .= sprintf(esc_html__($post_id) . '</p>');
			...
			//call Polylang plugin with appropriate language code
			if(function_exists('pll_set_post_language') && $params['event_language']['value']){
				//set the default language to English
				$language = 'en';
				switch($params['event_language']['value']){
					case 'Español':
						$language = 'es';
						break;
					case 'Tiếng Việt':
	...
	private function post_update( $post_id, $params, $id_field = 'ID', $post_type = '' ) {
		...
		if ( is_wp_error( $post_id ) ) {
			...
			//At this point we are assuming the post has been successfully updated
			//Let us set the language with the Polylang plugin
			//Polylang is loaded after this plugin, so let's load it if it has not already been loaded
			...
			$status = 'debug';
			$title  = sprintf(esc_html__( 'Updating a language', 'object-sync-for-salesforce' ), 
							  ucfirst( esc_attr( $status ) )
			);
